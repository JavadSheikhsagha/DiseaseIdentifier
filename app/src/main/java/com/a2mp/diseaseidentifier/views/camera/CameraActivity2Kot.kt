...
open class Camera2Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<View>(R.id.icon_flash).setOnClickListener { }
        val img = findViewById<ImageView>(R.id.img_snapTips)
        if (getSharedPreferences("camera", MODE_PRIVATE).getBoolean("firsttime", true)) {
            img.visibility = View.VISIBLE
            img.animate().alpha(0.0f).setDuration(700).setStartDelay(3000).start()
            getSharedPreferences("camera", MODE_PRIVATE).edit().putBoolean("firsttime", false)
                .apply()
        }
        imgGallery = findViewById(R.id.img_gallery)
        imgGallery?.setOnClickListener(View.OnClickListener { v: View? ->
    ...
    val captureCallbackListener: CaptureCallback = object : CaptureCallback() {
        override fun onCaptureCompleted(
            ...
            Toast.makeText(this@Camera2Activity, "ssss:$file", Toast.LENGTH_SHORT).show()
            createCameraPreview()
        }
    }
